...
export async function searchFlights(prefs) {
  const cacheKey = `${prefs.origin}-${prefs.destination}-${prefs.startDate}-${prefs.travelers}`;
...
function normalizeAmadeusOffer(offer) {
  ...
  const carrier = segments[0]?.carrierCode || offer.validatingAirlineCodes[0] || 'Unknown';
  const cabin = segments[0]?.cabin || 'ECONOMY';
  
  return {
    id: offer.id || `flight-${Date.now()}-${Math.random()}`,
    carrier: getAirlineName(carrier),
    segments,
