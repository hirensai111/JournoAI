...
/**
 * Generate itinerary for long trips by splitting into chunks
 */
async function generateLongTripItinerary(destination, startDate, endDate, days, destinationContext, preferences, flightInfo) {
  const allDays = [];
  const chunkSize = 7; // Generate 7 days at a time
  const numChunks = Math.ceil(days / chunkSize);

  console.log(`📅 Generating ${days}-day itinerary in ${numChunks} chunks of ${chunkSize} days each`);

  for (let chunk = 0; chunk < numChunks; chunk++) {
    const chunkStartDay = chunk * chunkSize;
    const chunkEndDay = Math.min((chunk + 1) * chunkSize, days);
    const chunkDays = chunkEndDay - chunkStartDay;

    // Calculate actual dates for this chunk
    const chunkStart = new Date(startDate);
    chunkStart.setDate(chunkStart.getDate() + chunkStartDay);

    const chunkEnd = new Date(startDate);
    chunkEnd.setDate(chunkEnd.getDate() + chunkEndDay - 1);

    console.log(`  📝 Chunk ${chunk + 1}/${numChunks}: Days ${chunkStartDay + 1}-${chunkEndDay} (${chunkDays} days)`);

    // Generate this chunk
    const chunkItinerary = await generateItineraryWithAI(
      destination,
      ...
      destinationContext,
      preferences,
      chunk === 0 ? flightInfo : null // Only first chunk needs flight arrival info
    );

    // Adjust day numbers
    for (const day of chunkItinerary) {
      day.day = (day.day || 1) + chunkStartDay;
      allDays.push(day);
    }
  }

  console.log(`✅ Combined all chunks into ${allDays.length}-day itinerary`);
  return allDays;
}
...
async function generateItineraryWithAI(destination, startDate, endDate, days, destinationContext, preferences, flightInfo) {
  ...
  // For trips longer than 10 days, split into chunks to avoid token limits
  if (days > 10) {
    console.log(`📅 Long trip detected (${days} days) - splitting into chunks`);
    return await generateLongTripItinerary(destination, startDate, endDate, days, destinationContext, preferences, flightInfo);
  }
...
2. Schedule 4-5 activities per day maximum (including meals)
3. Schedule activities with realistic timing (e.g., "2:00 PM: Visit Louvre Museum")
...
6. Include lunch and dinner at real restaurants
7. Add Google Maps-ready location names
8. IMPORTANT: Include the actual date for each day from the DATES list above
9. For each activity, include:
   - Exact time (e.g., "10:00 AM")
   - Full name of location
   - Clear, descriptive explanation (2-3 sentences) of what to see/do and why it's worth visiting
   - Activity type (museum/restaurant/park/landmark)
