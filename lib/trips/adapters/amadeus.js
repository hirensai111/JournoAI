...
function extractIATACode(location) {
  ...
  // If not found in mapping, return null to trigger error instead of defaulting to JFK
  if (!cityToIATA[normalized]) {
    console.warn(`⚠️  Unknown city: "${location}" - Please add IATA code to mapping`);
    return null;
  }
...
function generateMockFlights(prefs) {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const pricePerPerson = basePrice + Math.random() * 300;
    const price = pricePerPerson * travelers; // Total price for all travelers
