...
import { generateEnhancedItinerary } from './adapters/tripadvisor.js';
...
export async function composePlan(prefs) {
  ...
  // Try to generate enhanced itinerary using TripAdvisor + OpenAI
  let days;
  try {
    console.log('Attempting to generate enhanced itinerary with TripAdvisor + OpenAI...');
    const enhancedItinerary = await generateEnhancedItinerary(
      prefs.destination,
      prefs.startDate,
      prefs.endDate,
      {
        accessibility: prefs.travelersProfiles.flatMap(t => t.accessibilityNeeds || []),
        vibe: prefs.vibe,
        budget: prefs.budget,
        specialOccasion: prefs.specialOccasion,
        travelersProfiles: prefs.travelersProfiles
      }
    );

    if (enhancedItinerary && enhancedItinerary.length > 0) {
      console.log('✅ Using enhanced itinerary from TripAdvisor + OpenAI');
      days = enhancedItinerary;
    } else {
      console.log('⚠️  Enhanced itinerary empty, falling back to basic generation');
      days = buildDailyItinerary(prefs, rankedExperiences);
    }
  } catch (error) {
    console.error('❌ Enhanced itinerary generation failed:', error.message);
